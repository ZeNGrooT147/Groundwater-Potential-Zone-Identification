...
const MapViewer = () => {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true)
    setError('')
    setShowXAI(false) // Reset XAI when new search
  ...
  return (
    <div className="min-h-screen pt-32 pb-16 px-6 bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-[1800px] mx-auto relative z-10 px-4">
        ...
        >
          <h1 className="text-4xl font-black mb-3 bg-gradient-to-r from-white via-cyan-100 to-blue-200 bg-clip-text text-transparent drop-shadow-2xl">
            Interactive Groundwater Map
          </h1>
          <p className="text-slate-300 text-sm font-medium">
            Click on the map or enter coordinates • Dharwad District, Karnataka
          </p>
        </motion.div>
        
        {/* 3-Column Layout: Prediction | Map | XAI */}
        <div className="grid grid-cols-12 gap-4">
          ...
          >
            {result ? (
              <div className="bg-gradient-to-br from-slate-900/95 to-slate-800/95 backdrop-blur-xl border-2 border-slate-700/50 rounded-2xl shadow-2xl p-6 sticky top-6 max-h-[calc(100vh-120px)] overflow-y-auto">
                ...
                {/* Main GWP Result */}
                <div className={`${getGWPBg(result.prediction.groundwater_class)} border-2 rounded-2xl p-6 mb-6 text-center`}>
                  <p className="text-xs text-slate-300 uppercase font-bold tracking-wider mb-2">Groundwater Potential</p>
                  <p className={`text-5xl font-black mb-2 ${getGWPColor(result.prediction.groundwater_class)}`}>
                    {result.prediction.groundwater_class}
                  </p>
                  <div className="flex items-center justify-center gap-2 text-sm">
                    <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                    <span className="text-slate-300 font-medium">{(result.prediction.confidence * 100).toFixed(1)}% Confidence</span>
                  </div>
                </div>

                {/* Environmental Indicators - Compact */}
                <div className="space-y-3 mb-6">
                  <h3 className="text-xs text-slate-400 uppercase font-bold tracking-wider mb-3">Environmental Indicators</h3>
                  
                  {/* NDVI */}
                  <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-slate-300 font-medium">Vegetation (NDVI)</span>
                      <span className="text-lg font-black text-green-400">{result.prediction.ndvi.toFixed(3)}</span>
                    </div>
                    <div className="w-full bg-slate-800 rounded-full h-1.5">
                      <div className="bg-gradient-to-r from-green-600 to-green-400 h-1.5 rounded-full" style={{ width: `${Math.min(100, (result.prediction.ndvi + 1) * 50)}%` }}></div>
                    </div>
                  </div>
                  ...
                  <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-slate-300 font-medium">Water Content (NDWI)</span>
                      <span className="text-lg font-black text-blue-400">{result.prediction.ndwi.toFixed(3)}</span>
                    </div>
                    <div className="w-full bg-slate-800 rounded-full h-1.5">
                      <div className="bg-gradient-to-r from-blue-600 to-blue-400 h-1.5 rounded-full" style={{ width: `${Math.min(100, (result.prediction.ndwi + 1) * 50)}%` }}></div>
                    </div>
                  </div>
                  ...
                  <div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-slate-300 font-medium">Elevation (DEM)</span>
                      <span className="text-lg font-black text-purple-400">{result.prediction.elevation.toFixed(0)}m</span>
                    </div>
                    <div className="w-full bg-slate-800 rounded-full h-1.5">
                      <div className="bg-gradient-to-r from-purple-600 to-purple-400 h-1.5 rounded-full" style={{ width: `${Math.min(100, (result.prediction.elevation / 1000) * 100)}%` }}></div>
                    </div>
                  </div>
                </div>

                {/* AI Analysis Summary */}
                <div className="bg-slate-800/40 border border-slate-700/30 rounded-lg p-4 mb-4">
                  ...
                  <div className="space-y-1.5">
                    {result.explanation.factors.slice(0, 3).map((factor, index) => (
                ...
                {/* Coordinates */}
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-800/40 rounded-lg border border-slate-700/30 mb-4">
                  <MapPin className="w-4 h-4 text-cyan-400" />
                  <span className="text-xs text-slate-300">
                    <span className="font-bold text-white">{result.location.lat.toFixed(4)}°N</span>, <span className="font-bold text-white">{result.location.lon.toFixed(4)}°E</span>
                  </span>
                </div>
                ...
                {result.xai && (
                  <button
                    ...
                    className={`w-full px-4 py-3 rounded-xl font-bold text-white flex items-center justify-center gap-2 transition-all text-sm border-2 ${
                      showXAI 
                        ? 'bg-gradient-to-r from-indigo-600 to-violet-600 border-indigo-500/50 shadow-lg shadow-indigo-500/30' 
                        : 'bg-slate-700/50 border-slate-600/50 hover:border-indigo-500/50'
                    }`}
                  >
                    <Brain className="w-4 h-4" />
                    {showXAI ? 'Hide XAI Analysis →' : 'Show XAI Analysis →'}
                  </button>
                )}
              </div>
            ) : (
              <div className="bg-gradient-to-br from-slate-900/95 to-slate-800/95 backdrop-blur-xl border-2 border-slate-700/50 rounded-2xl shadow-2xl p-6">
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-bold text-white mb-2">No Prediction Yet</h3>
                  <p className="text-sm text-slate-400">Click on the map or enter coordinates to get started</p>
                </div>
              </div>
            )}
          </motion.div>
          ...
          >
            {/* Coordinate Search */}
            <div className="bg-gradient-to-br from-slate-900/95 to-slate-800/95 backdrop-blur-xl border-2 border-slate-700/50 p-8 rounded-2xl shadow-2xl">
              <div className="flex items-center gap-4 mb-8">
                ...
                <h2 className="text-xl font-bold bg-gradient-to-r from-white to-cyan-100 bg-clip-text text-transparent">Search Location</h2>
              </div>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-5 py-3.5 bg-slate-800/60 border-2 border-slate-600/50 rounded-xl focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/30 transition-all text-white placeholder-slate-400 font-medium hover:border-slate-500/70"
                  />
                </div>
                ...
                {error && (
                  <motion.div 
                    ...
                    className="p-4 bg-gradient-to-r from-red-500/20 to-red-600/10 border-2 border-red-500/40 rounded-xl text-red-200 text-sm flex items-start gap-3 shadow-lg shadow-red-500/10"
                  >
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full px-6 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 rounded-xl font-bold text-white flex items-center justify-center gap-3 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-cyan-500/30 hover:shadow-xl hover:shadow-cyan-500/40 hover:scale-[1.02] text-base"
                >
                  {loading ? (
                    <>
                ...
                {/* Back to Dharwad Button */}
                <button
                  onClick={handleBackToDharwad}
                  className="w-full px-6 py-3 bg-gradient-to-r from-slate-700 to-slate-600 hover:from-slate-600 hover:to-slate-500 rounded-xl font-bold text-white flex items-center justify-center gap-3 transition-all shadow-lg hover:shadow-xl hover:scale-[1.02] text-sm border-2 border-slate-500/50"
                >
                  <Target className="w-4 h-4" />
                  Back to Dharwad Center
                </button>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-slate-900/95 to-slate-800/95 backdrop-blur-xl border-2 border-slate-700/50 p-8 rounded-2xl shadow-2xl">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-bold bg-gradient-to-r from-white to-cyan-100 bg-clip-text text-transparent">District Info</h3>
              </div>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center p-4 bg-gradient-to-r from-slate-800/60 to-slate-900/60 rounded-xl border-2 border-slate-700/30 hover:border-slate-600/50 transition-all group">
                  <span className="text-slate-300 font-medium">Latitude Range</span>
                  <span className="font-bold text-white group-hover:text-cyan-300 transition-colors">15.0° - 16.0° N</span>
                </div>
                <div className="flex justify-between items-center p-4 bg-gradient-to-r from-slate-800/60 to-slate-900/60 rounded-xl border-2 border-slate-700/30 hover:border-slate-600/50 transition-all group">
                  <span className="text-slate-300 font-medium">Longitude Range</span>
                  <span className="font-bold text-white group-hover:text-cyan-300 transition-colors">74.5° - 75.5° E</span>
                </div>
                <div className="flex justify-between items-center p-4 bg-gradient-to-r from-slate-800/60 to-slate-900/60 rounded-xl border-2 border-slate-700/30 hover:border-slate-600/50 transition-all group">
                  <span className="text-slate-300 font-medium">Total Area</span>
                  <span className="font-bold text-white group-hover:text-cyan-300 transition-colors">~4,260 km²</span>
                </div>
                <div className="flex justify-between items-center p-4 bg-gradient-to-r from-cyan-500/20 to-blue-500/10 rounded-xl border-2 border-cyan-500/40 shadow-lg shadow-cyan-500/10 hover:shadow-cyan-500/20 transition-all group">
                  <span className="text-cyan-200 font-bold">Model Accuracy</span>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-cyan-400 rounded-full animate-pulse"></div>
                    <span className="font-black text-cyan-300 group-hover:text-cyan-200 transition-colors">High Precision</span>
                  </div>
                </div>
              </div>
            </div>
